...
loglikProbBB <- function(beta, death.remain, report.new, X){
  ...
  return(list(loglik = lik, grad = c(grad_alpha,grad_beta)))
}
##
...
sample.deathsBB <- function(samples, deaths, alpha, beta, Reported, alpha.MCMC, rep.day = Inf){
  ...
  for(i in 1:N){
    if(i+rep.day <=N){
      ...
      lik_i <- loglikDeathsGivenProbBB(deaths[i], alpha[i,i:N], beta[i,i:N], Reported[i,i:N])
      for(j in 1:samples){
        death_star <- sample((deaths[i]-alpha.MCMC[i]):(deaths[i]+alpha.MCMC[i]), 1)
        lik_start <- loglikDeathsGivenProbBB(death_star, alpha[i,i:N], beta[i,i:N], Reported[i,i:N])
        if(log(runif(1)) < lik_start-lik_i){
          lik_i = lik_start
          deaths[i] <- death_star
          acc[i] = acc[i] + 1
